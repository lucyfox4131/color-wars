    ...
    blocks.forEach(function(block){
      if (block.contains({x: click.x, y: (click.y)})){
        ...
        if (block.color === "blue"){
          ...
          var userFiller = new ColorFiller(block, pattern, context);
          userFiller.expandColor(block, context);
          var computerFiller = new ColorFiller(compBlock, pattern, context);
          computerFiller.expandColor(compBlock, context);
