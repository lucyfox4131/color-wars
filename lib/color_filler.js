...
function calculateColorValues(game, userColor){
  ...
  drawScoreCounters(compBlocks[0].color, 10, compCount1/blocks.length)
  drawScoreCounters(compBlocks[1].color, 30, compCount2/blocks.length)
  drawScoreCounters(userColor, 50, userCount/blocks.length)
...
function drawScoreCounters(color, y, percent){
  fillContext.beginPath();
  fillContext.fillStyle = color;
  fillContext.rect(0, y, Math.round(fillCanvas.width*(percent)), 10);
  fillContext.fill();
}
