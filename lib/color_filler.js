...
ColorFiller.prototype.expandColor = function(currentBlock, context, compColor, userColor, game){
  ...
  if (currentGreyParent){
    parent = changeNeighborColor(parent, currentBlock)
    fillInNeighborBlock.call(this, parent, currentBlock, context, compColor, userColor, game);
  }
  if(currentGreyChild){
    child = changeNeighborColor(child, currentBlock)
...
function changeNeighborColor(neighbor, currentBlock){
  neighbor.color = currentBlock.color;
  return neighbor;
}
